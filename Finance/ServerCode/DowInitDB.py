...
def add2DB(df, db, stock):
	...
	cursor.execute("CREATE TABLE " + stock + "(Date Date, Open float, High float, Low float, Close float, AdjClose float, SplitCoeff int, MACD float, MACDhist float, MACDsignal float, RSI float, WILLR float, ADX float, MOM float)")
